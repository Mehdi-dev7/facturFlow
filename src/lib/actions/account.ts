...
// ─── Sauvegarder la couleur de texte du header ───────────────────────────────

export async function saveHeaderTextColor(headerTextColor: string) {
  const session = await auth.api.getSession({ headers: await headers() });
  if (!session) return { success: false, error: "Non authentifié" } as const;

  const allowed = ["auto", "white", "black"];
  if (!allowed.includes(headerTextColor)) {
    return { success: false, error: "Valeur invalide" } as const;
  }
  ...
  try {
    ...
    return { success: false, error: "Impossible de sauvegarder" } as const;
  }
}
