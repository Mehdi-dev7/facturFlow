...
export async function createDepositFromQuote(quoteId: string, userId: string) {
  try {
    ...
    // Détecter les providers de paiement actifs pour inclure les boutons dans l'email
...
export async function updateDeposit(id: string, data: DepositFormData) {
  ...
  try {
    ...
    const customNumber =
      rawCustomNumber &&
      rawCustomNumber !== existingDeposit.number &&
    ...
    if (customNumber) {
      const duplicate = await prisma.document.findFirst({
        where: { userId: session.user.id, number: customNumber, id: { not: id } },
      });
      if (duplicate) {
    ...
    // Finaliser un brouillon : générer un vrai numéro DEP-AAAA-XXXX
    let finalizedNumber: string | undefined;
    if (existingDeposit.number.startsWith("BROUILLON-") && !customNumber) {
      const updatedUser = await prisma.user.update({
