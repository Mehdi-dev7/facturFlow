...
import { useUpdateDepositStatus } from "@/hooks/use-deposits";
...
const TRANSITIONS: Record<string, Transition[]> = {
  DRAFT: [
    ...
    { status: "PAID",  label: "Marquer comme payé",     color: "text-emerald-500" },
  ],
  SENT: [
    { status: "PAID",    label: "Marquer comme payé",     color: "text-emerald-500" },
    { status: "OVERDUE", label: "Marquer comme impayé",   color: "text-orange-500" },
  ],
  OVERDUE: [
    { status: "PAID",     label: "Marquer comme payé",     color: "text-emerald-500" },
    { status: "REMINDED", label: "Marquer comme relancé",  color: "text-red-500" },
  ],
  REMINDED: [
    { status: "PAID", label: "Marquer comme payé", color: "text-emerald-500" },
  ],
  ...
  PAID: [
    ...
    { status: "OVERDUE", label: "Annuler → Impayé",  color: "text-orange-500" },
  ],
};

// ─── Composant ────────────────────────────────────────────────────────────────

interface StatusDropdownDepositProps {
  depositId: string;
  dbStatus: string;
}

export function StatusDropdownDeposit({ depositId, dbStatus }: StatusDropdownDepositProps) {
  const updateStatus = useUpdateDepositStatus();
          ...
          >
            ...
            {transitions.map((t) => (
              <DropdownMenuPrimitive.Item
                key={t.status}
                onSelect={() => updateStatus.mutate({ id: depositId, status: t.status })}
