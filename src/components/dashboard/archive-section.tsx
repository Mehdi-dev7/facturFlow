...
export function ArchiveSection({ data, onSelect }: ArchiveSectionProps) {
  ...
  return (
    <div className="rounded-2xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-[#1a1438] backdrop-blur-lg overflow-hidden">
      <div className="px-4 sm:px-6 py-3 sm:py-4 border-b border-slate-200 dark:border-slate-600">
          ...
          return (
            <div key={yearData.year}>
              <button
                ...
                className="w-full flex items-center justify-between px-4 sm:px-6 py-3 hover:bg-violet-200/30 dark:hover:bg-violet-500/30 transition-colors cursor-pointer"
              >
                  ...
                  <div className="px-4 sm:px-6 pb-3 flex flex-wrap gap-2 mt-3">
                    {yearData.months.map((m) => (
                      <button
                        ...
                        className="px-3 py-1.5 rounded-lg text-xs font-medium bg-violet-100 dark:bg-violet-500/25 text-violet-700 dark:text-violet-300 hover:bg-violet-200 dark:hover:bg-violet-500/25 transition-colors cursor-pointer"
                      >
                        {m.month} ({m.count})
                      </button>
                    ))}
