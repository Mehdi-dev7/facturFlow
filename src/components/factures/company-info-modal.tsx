		...
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className="sm:max-w-md bg-linear-to-b from-violet-50 via-white to-white dark:from-[#2a2254] dark:via-[#221c48] dark:to-[#221c48] border border-primary/20 dark:border-violet-400/25 shadow-lg dark:shadow-violet-950/40 rounded-xl">
				...
				<form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
					<div>
						<Label htmlFor="companyName" className="text-slate-700 dark:text-violet-200">Nom de l&apos;entreprise *</Label>
						<Input
							id="companyName"
							{...register("name")}
					...
					<div>
						<Label htmlFor="companySiret" className="text-slate-700 dark:text-violet-200">SIRET *</Label>
						<Input
							id="companySiret"
							{...register("siret")}
						...
						/>
						{errors.siret && <p className="text-xs text-red-500 dark:text-red-400 mt-1">{errors.siret.message}</p>}
					</div>
					<div className="grid grid-cols-2 gap-3">
						<div>
							<Label htmlFor="companyAddress" className="text-slate-700 dark:text-violet-200">Adresse *</Label>
							<Input
								id="companyAddress"
								{...register("address")}
								className="bg-white/90 dark:bg-[#2a2254] border-slate-300 dark:border-violet-400/30 rounded-xl text-slate-900 dark:text-slate-50"
							...
							/>
							{errors.address && <p className="text-xs text-red-500 dark:text-red-400 mt-1">{errors.address.message}</p>}
						</div>
						<div>
							<Label htmlFor="companyCity" className="text-slate-700 dark:text-violet-200">Ville *</Label>
							<Input
								id="companyCity"
								{...register("city")}
								className="bg-white/90 dark:bg-[#2a2254] border-slate-300 dark:border-violet-400/30 rounded-xl text-slate-900 dark:text-slate-50"
							...
							/>
							{errors.city && <p className="text-xs text-red-500 dark:text-red-400 mt-1">{errors.city.message}</p>}
						</div>
					</div>
					<div>
						<Label htmlFor="companyEmail" className="text-slate-700 dark:text-violet-200">Email *</Label>
						<Input
							id="companyEmail"
							type="email"
							{...register("email")}
					...
					<div>
						<Label htmlFor="companyPhone" className="text-slate-700 dark:text-violet-200">Téléphone</Label>
						<Input
							id="companyPhone"
							type="tel"
							{...register("phone")}
