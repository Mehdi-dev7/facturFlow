...
export default function EditProformaPage() {
	...
	const getDocumentForPreview = useCallback(() => {
		...
		const mock = buildPreviewInvoice(values, proforma?.number ?? "", companyInfo, { themeColor, companyFont, companyLogo, invoiceFooter , currency, headerTextColor}, clients);
	...
	return (
		<div>
			...
			{/* Mobile : stepper */}
			<div className="lg:hidden rounded-2xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-[#1a1438] backdrop-blur-lg shadow-lg shadow-slate-300/70 dark:shadow-violet-950/40 min-h-[70vh]">
				<InvoiceStepper
					form={form}
					onSubmit={onSubmit}
					invoiceNumber={displayNumber || proforma?.number || ""}
